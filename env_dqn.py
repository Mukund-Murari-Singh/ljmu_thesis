...
class market():
    ...
    #Create function to fetch the next day's market vector given a vector from ms_dict without OHQ, Cash & Price_Buy
    def next_market_vector(self, state):#ms_vec): [tuple,array, timestamp]
        #tup_ms_vec=tuple([tuple(vector) for vector in ms_vec])
    ...
    #This will be a 2 dimensional array representing the market_state; 
    #For this, i need the market_vector & the cash-on-hand
    #This needs the output of action_success function to be fed into next_internal_state function
    #action_success function implements the action on current state to fetch reward and next_state
    #reward is used to calculate cash_on_hand and other parameters
    
    def state_encoder(self, state): #state is [tuple,array]
        """convert the state into a vector so that it can be fed to the NN. This method combines ms_vect with internal state of Cash and transforms it into a 2D array"""
        #ms_encod=np.array(state[0][training_cols])
        #int_vec_encod=state[1]
        state_encod=np.array(state[0][training_cols])#np.hstack((ms_encod,int_vec_encod))
        #state_encod=state_encod.reshape(state_encod.shape[0],state_encod.shape[1],1) conv1D doesn't need 3 channels like conv2d input
        #tup_state_encod=tuple([tuple(vector) for vector in state_encode])
        return state_encod
    ...
    def action_success_fn(self,action,state): #state is [tuple,array, timestamp]
        ...
        action_tkr=self.get_ticker_from_action(action)
        ms_vec1=state[0] 
        ...
        if ms_vec1.Date[0] in sell_dict.keys():
            ...
            #CHECK THE ABOVE CODE AND THEN MODIFY THIS FUNCTION CODE
        ...
        if action_tkr!=-1:
            ...
            #Update cash_on_hand only upto the initial capital since we are not reinvesting profits           
            #new_cash_on_hand=0.0
            #new_cash_on_hand=min(coh_today+step_reward,initial_cash) 
            #cash_on_hand=new_cash_on_hand
        ...
        return next_state, step_reward, step_dividend,trig_count
